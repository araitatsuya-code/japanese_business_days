...
class PerformanceBenchmark
  ...
  def run_basic_operations_benchmark
    ...
    @results[:basic_operations] = BenchmarkHelper.compare_benchmarks(benchmarks, 10000)
  end
  ...
  def run_scalability_benchmark
    ...
    periods.each do |name, start_date, end_date|
      ...
      @results["period_#{name.gsub(' ', '_')}"] = result
    end
  end
  ...
  def run_memory_benchmark
    ...
    memory_tests.each do |name, test|
      ...
      @results["memory_#{name.gsub(' ', '_').gsub('?', '')}"] = memory_usage
    end
  end
  ...
  def run_real_world_scenarios
    ...
    scenarios = {
      ...
      "Financial settlement processing" => -> {
        ...
        12.times do |month|
          ...
          # 営業日でない場合は前営業日
          settlement_date = JapaneseBusinessDays.business_day?(month_end) ? 
                           month_end : 
      ...
      "Delivery scheduling" => -> {
        ...
        order_dates.each do |order_date|
          ...
          processing_date = JapaneseBusinessDays.business_day?(order_date) ?
                           order_date :
                           JapaneseBusinessDays.next_business_day(order_date)
          
          # 3営業日後に配送
          delivery_date = JapaneseBusinessDays.add_business_days(processing_date, 3)
        end
      }
    }
    ...
    scenarios.each do |name, scenario|
      ...
      @results["scenario_#{name.gsub(' ', '_').downcase}"] = result
    end
  end
  ...
  def print_summary
    ...
    @results.select { |k, _| k.to_s.start_with?('memory_') }.each do |key, result|
      name = key.to_s.gsub('memory_', '').gsub('_', ' ')
    ...
    @results.select { |k, _| k.to_s.start_with?('scenario_') }.each do |key, result|
      name = key.to_s.gsub('scenario_', '').gsub('_', ' ').split.map(&:capitalize).join(' ')
