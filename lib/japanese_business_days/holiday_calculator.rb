...
module JapaneseBusinessDays
  ...
  class HolidayCalculator
    ...
    def happy_monday_holiday?(date)
      ...
      HAPPY_MONDAY_HOLIDAYS.each do |(month, nth_week), _name|
        if date.month == month
    ...
    def substitute_holidays_in_year(year, existing_holidays)
      substitute_holidays = []
      ...
      existing_holidays.each do |holiday|
        ...
        if holiday.date.sunday?
          # 日曜日の祝日の翌日（月曜日）を振替休日とする
          substitute_date = holiday.date + 1
          
          # 振替休日が既存の祝日と重複しないかチェック
          unless existing_holidays.any? { |h| h.date == substitute_date }
            substitute_holidays << Holiday.new(substitute_date, "振替休日", :substitute)
          end
        end
      end
    ...
    def nth_weekday(year, month, nth, weekday)
      ...
      if target_date.month != month
        raise ArgumentError, "The #{nth}#{ordinal_suffix(nth)} #{weekday_name(weekday)} of #{month}/#{year} does not exist"
      end
