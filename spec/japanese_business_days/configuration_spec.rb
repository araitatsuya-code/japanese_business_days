  ...
  describe '#additional_holidays=' do
    ...
    context 'with invalid input' do
      it 'raises InvalidArgumentError for non-array' do
        ...
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /additional_holidays must be an Array/)
      end

      it 'raises InvalidArgumentError for array with non-Date objects' do
        expect {
          config.additional_holidays = [Date.new(2024, 1, 1), "2024-01-02"]
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /additional_holidays\[1\] must be a Date object/)
      end
  ...
  describe '#additional_business_days=' do
    ...
    context 'with invalid input' do
      it 'raises InvalidArgumentError for non-array' do
        ...
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /additional_business_days must be an Array/)
      end

      it 'raises InvalidArgumentError for array with non-Date objects' do
        expect {
          config.additional_business_days = [Date.new(2024, 1, 1), Time.now]
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /additional_business_days\[1\] must be a Date object/)
      end
    end
  end
  ...
  describe '#weekend_days=' do
    ...
    context 'with invalid input' do
      ...
      it 'raises InvalidArgumentError for empty array' do
        expect {
          config.weekend_days = []
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /weekend_days cannot be empty/)
      end

      it 'raises InvalidArgumentError for invalid weekday numbers' do
        expect {
          config.weekend_days = [0, 7] # 7 is invalid
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /weekend_days\[1\] must be an integer between 0-6/)
      end

      it 'raises InvalidArgumentError for negative numbers' do
        expect {
          config.weekend_days = [-1, 0]
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /weekend_days\[0\] must be an integer between 0-6/)
      end
      ...
      it 'raises InvalidArgumentError for duplicate values' do
        ...
        }.to raise_error(JapaneseBusinessDays::InvalidArgumentError, /weekend_days cannot contain duplicate values/)
      end
    end
  end
